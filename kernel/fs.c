...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT * NINDIRECT){
    // Allocate doubly indirect block if missing
...
// [MODIFIED] itrunc() now handles doubly-indirect freeing
void
itrunc(struct inode *ip)
{
  struct buf *bp1, *bp2;
...
symlink(const char *target, const char *path)
{
    ...
    // Get parent directory of the symlink
    ...
    // Add symlink entry in parent directory
